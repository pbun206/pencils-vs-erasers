...
fn main() -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([SIDEBAR + COLS as f32 * CELL_W, TOP_BAR + LANES as f32 * CELL_H])
            .with_resizable(false),
        ..Default::default()
    };
    eframe::run_native("Pencils vs Erasers", options, Box::new(|_cc| Ok(Box::new(Game::new()))))
}
...
fn main() {
    ...
    wasm_bindgen_futures::spawn_local(async {
        ...
        eframe::WebRunner::new()
            .start(canvas, web_options, Box::new(|_cc| Ok(Box::new(Game::new()))))
            .await
...
enum DefenderKind { Pencil, Notebook, Highlighter, Pen, Marker }

impl DefenderKind {
    fn cost(self) -> i32 {
        match self {
            Self::Pencil => 100, Self::Notebook => 50, Self::Highlighter => 125,
            Self::Pen => 200, Self::Marker => 600,
        }
    }
    fn hp(self) -> f32 {
        match self {
            Self::Pencil => 80.0, Self::Notebook => 300.0, Self::Highlighter => 60.0,
            Self::Pen => 70.0, Self::Marker => 50.0,
        }
    }
    ...
    fn label(self) -> &'static str {
        match self {
            Self::Pencil => "Pencil", Self::Notebook => "Notebook",
            Self::Highlighter => "Highlight", Self::Pen => "Pen", Self::Marker => "Marker",
        }
    }
    fn symbol(self) -> &'static str {
        match self { Self::Pencil => "/", Self::Notebook => "[]", Self::Highlighter => "//", Self::Pen => "|", Self::Marker => "!" }
    }
}

struct Defender {
    kind: DefenderKind, lane: usize, col: usize,
    hp: f32, attack_timer: f32, attack_flash: f32,
}
impl Defender {
    fn new(kind: DefenderKind, lane: usize, col: usize) -> Self {
        Self { kind, lane, col, hp: kind.hp(), attack_timer: 0.0, attack_flash: 0.0 }
    }
}

struct Projectile {
    lane: usize, x: f32, damage: f32, pierce: bool, hits: usize, is_highlighter: bool,
}
...
enum EnemyKind {
    PinkEraser, WhitePolymer, WrappedPolymer,
    BlackEraser, WrappedBlackEraser, ElectronicEraser,
    EraserHolder, BlueEraserHolder,
    Scissors, BlueScissors,
    WhiteOut,
    KneadedEraser,
    ShinyPlastic, LargeShinyPlastic,
}
...
impl EnemyKind {
    ...
    fn is_scissors(self) -> bool { matches!(self, Self::Scissors | Self::BlueScissors) }
    fn instant_kills(self) -> bool { matches!(self, Self::Scissors | Self::BlueScissors | Self::WhiteOut | Self::ElectronicEraser) }
    fn is_holder(self) -> bool { matches!(self, Self::EraserHolder | Self::BlueEraserHolder) }
    fn is_shiny(self) -> bool { matches!(self, Self::ShinyPlastic | Self::LargeShinyPlastic) }
    fn is_black(self) -> bool { matches!(self, Self::BlackEraser | Self::WrappedBlackEraser) }
...
enum EnemyState {
    Moving, Eating,
    HolderStopping(f32), HolderExtending, HolderAttacking,
    Retreating,
}

struct Enemy {
    kind: EnemyKind, lane: usize, x: f32,
    hp: f32, shell_hp: f32, speed: f32, damage: f32,
    attack_timer: f32, attack_flash: f32, hit_flash: f32,
    state: EnemyState, has_jumped: bool, should_jump: bool,
    extend_offset: f32, extend_target: f32,
}
...
impl Enemy {
    fn new(kind: EnemyKind, lane: usize) -> Self {
        Self {
            ...
            hp: kind.base_hp(), shell_hp: kind.shell_hp(),
            speed: kind.base_speed(), damage: kind.base_damage(),
            attack_timer: 0.0, attack_flash: 0.0, hit_flash: 0.0,
            state: EnemyState::Moving, has_jumped: false, should_jump: false,
            extend_offset: 0.0, extend_target: 0.0,
        }
    }

    fn take_damage(&mut self, amount: f32, from_highlighter: bool) {
        if self.kind.is_shiny() && from_highlighter { return; }
...
impl PathwayLevel {
    fn pick(&self, rng: &mut impl Rng) -> EnemyKind {
        match self {
            Self::Single(k) => *k,
            Self::CoinFlip(a, b) => if rng.random_range(0.0_f32..1.0) < 0.5 { *a } else { *b },
        }
    }
}
...
fn create_pathways() -> Vec<Pathway> {
    vec![
        ...
        // E: KneadedEraser
        Pathway { levels: vec![
            PathwayLevel::Single(EnemyKind::KneadedEraser),
        ]},
...
#[derive(Clone, Copy, PartialEq)]
enum Tool { Place(DefenderKind), InkBlob, PencilCase }
...
fn highscore_path() -> PathBuf {
    let mut p = std::env::current_exe().unwrap_or_default().parent().unwrap_or(std::path::Path::new(".")).to_path_buf();
...
fn load_highscore() -> Option<f64> {
    std::fs::read_to_string(highscore_path()).ok()?.trim().parse().ok()
}
...
#[cfg(target_arch = "wasm32")]
fn load_highscore() -> Option<f64> { None }
...
impl Game {
    fn new() -> Self {
        ...
        Self {
            defenders: Vec::new(), projectiles: Vec::new(), enemies: Vec::new(),
            ink: 200, ink_timer: 0.0,
            selected: Some(Tool::Place(DefenderKind::Pencil)),
            wave: 1, state: GameState::Playing, time: 0.0,
            ink_blob_available: true, bonus_msg_timer: 0.0, bonus_msg_amount: 0,
            pathways, pathway_introduced,
            pathway_level: vec![0; num_p],
            miniwave: 0, miniwave_timer: 0.0,
            wallet: 0.0,
    ...
    fn pick_weighted_pathway(&self, rng: &mut impl Rng, allow_invest: bool) -> Option<usize> {
        ...
        for i in 0..self.pathways.len() {
            if self.pathway_introduced[i] {
                let w = if self.boosted_pathways.contains(&i) { 2.0 } else { 1.0 };
    ...
    fn process_miniwave(&mut self) {
        ...
        if self.miniwave == 0 && !self.start_intro_done {
            ...
            if self.wave == 1 {
                ...
                if self.coin_was_heads {
                    ...
                    let others: Vec<usize> = upgradeable.into_iter().filter(|&p| p != r_pidx).collect();
                    if !others.is_empty() {
        ...
        loop {
            ...
            match self.pick_weighted_pathway(&mut rng, allow_invest) {
                None => {
                    if allow_invest { self.invested = true; }
                    break;
                }
                Some(pidx) => {
                    first_pick = false;
    ...
    fn draw_defender_sprite(painter: &egui::Painter, kind: DefenderKind, c: egui::Pos2, scale: f32) {
        let s = scale;
        match kind {
            DefenderKind::Pencil => {
                let body = egui::Rect::from_min_size(
                    egui::pos2(c.x - 18.0 * s, c.y - 5.0 * s), egui::vec2(28.0 * s, 10.0 * s));
                painter.rect_filled(body, 1.0 * s, egui::Color32::from_rgb(220, 200, 60));
                painter.rect_stroke(body, 1.0 * s, egui::Stroke::new(0.5, egui::Color32::from_rgb(180, 160, 40)), egui::StrokeKind::Outside);
                let tip = vec![
                    egui::pos2(body.max.x, c.y - 5.0 * s),
                    egui::pos2(body.max.x + 12.0 * s, c.y),
                    egui::pos2(body.max.x, c.y + 5.0 * s),
                ];
                painter.add(egui::Shape::convex_polygon(tip, egui::Color32::from_rgb(210, 180, 140), egui::Stroke::NONE));
                let gpt = vec![
                    egui::pos2(body.max.x + 8.0 * s, c.y - 2.0 * s),
                    egui::pos2(body.max.x + 12.0 * s, c.y),
                    egui::pos2(body.max.x + 8.0 * s, c.y + 2.0 * s),
                ];
                painter.add(egui::Shape::convex_polygon(gpt, egui::Color32::from_rgb(60, 60, 60), egui::Stroke::NONE));
                let eraser = egui::Rect::from_min_size(
                    egui::pos2(body.min.x - 6.0 * s, c.y - 5.0 * s), egui::vec2(6.0 * s, 10.0 * s));
                painter.rect_filled(eraser, 1.0 * s, egui::Color32::from_rgb(240, 150, 150));
                let band = egui::Rect::from_min_size(
                    egui::pos2(body.min.x - 1.0 * s, c.y - 5.0 * s), egui::vec2(3.0 * s, 10.0 * s));
                painter.rect_filled(band, 0.0, egui::Color32::from_rgb(180, 180, 190));
            }
            DefenderKind::Pen => {
                let body = egui::Rect::from_min_size(
                    egui::pos2(c.x - 18.0 * s, c.y - 4.0 * s), egui::vec2(30.0 * s, 8.0 * s));
                painter.rect_filled(body, 1.0 * s, egui::Color32::from_rgb(25, 25, 35));
                painter.rect_stroke(body, 1.0 * s, egui::Stroke::new(0.5, egui::Color32::from_rgb(60, 60, 70)), egui::StrokeKind::Outside);
                let tip = vec![
                ...
                painter.add(egui::Shape::convex_polygon(tip, egui::Color32::from_rgb(50, 50, 60), egui::Stroke::NONE));
                painter.circle_filled(egui::pos2(body.max.x + 14.0 * s, c.y), 1.5 * s, egui::Color32::from_rgb(180, 180, 190));
                painter.line_segment(
                    [egui::pos2(c.x - 8.0 * s, c.y - 4.0 * s), egui::pos2(c.x + 8.0 * s, c.y - 4.0 * s)],
                    egui::Stroke::new(2.0 * s, egui::Color32::from_rgb(180, 180, 190)));
                let cap = egui::Rect::from_min_size(
                    egui::pos2(body.min.x - 4.0 * s, c.y - 4.0 * s), egui::vec2(4.0 * s, 8.0 * s));
            ...
            DefenderKind::Notebook => {
                ...
                painter.rect_stroke(r, 3.0 * s, egui::Stroke::new(1.5 * s, egui::Color32::from_rgb(60, 90, 150)), egui::StrokeKind::Outside);
                for j in 0..3 {
                    let y = r.min.y + 10.0 * s + j as f32 * 10.0 * s;
                    painter.line_segment([egui::pos2(r.min.x + 5.0 * s, y), egui::pos2(r.max.x - 5.0 * s, y)],
            ...
            DefenderKind::Highlighter => {
                ...
                painter.rect_stroke(r, 2.0 * s, egui::Stroke::new(0.5, egui::Color32::from_rgb(180, 210, 30)), egui::StrokeKind::Outside);
                // Chisel tip
                let pts = vec![
                ...
                painter.add(egui::Shape::convex_polygon(pts, egui::Color32::from_rgb(200, 240, 30), egui::Stroke::NONE));
                // Cap end
                let cap = egui::Rect::from_min_size(
                    egui::pos2(r.min.x - 4.0 * s, c.y - 5.0 * s), egui::vec2(4.0 * s, 10.0 * s));
                painter.rect_filled(cap, 1.0 * s, egui::Color32::from_rgb(180, 210, 30));
            }
            DefenderKind::Marker => {
                // Dark body (like a Sharpie)
                let body = egui::Rect::from_min_size(
                    egui::pos2(c.x - 16.0 * s, c.y - 5.0 * s), egui::vec2(26.0 * s, 10.0 * s));
                painter.rect_filled(body, 2.0 * s, egui::Color32::from_rgb(40, 40, 45));
                painter.rect_stroke(body, 2.0 * s, egui::Stroke::new(0.5, egui::Color32::from_rgb(25, 25, 30)), egui::StrokeKind::Outside);
                // Tapered tip section
                let tip = vec![
                    egui::pos2(body.max.x, c.y - 5.0 * s),
                ...
                painter.add(egui::Shape::convex_polygon(tip, egui::Color32::from_rgb(50, 50, 55), egui::Stroke::NONE));
                // Felt tip
                let felt = egui::Rect::from_min_size(
                    egui::pos2(body.max.x + 8.0 * s, c.y - 2.0 * s), egui::vec2(4.0 * s, 4.0 * s));
                painter.rect_filled(felt, 0.5 * s, egui::Color32::from_rgb(70, 70, 75));
                // Red cap on the back
                let cap = egui::Rect::from_min_size(
                    egui::pos2(body.min.x - 6.0 * s, c.y - 6.0 * s), egui::vec2(6.0 * s, 12.0 * s));
                painter.rect_filled(cap, 2.0 * s, egui::Color32::from_rgb(200, 40, 40));
                // Red band/ring on body
                let band = egui::Rect::from_min_size(
                    egui::pos2(body.min.x + 2.0 * s, c.y - 5.0 * s), egui::vec2(4.0 * s, 10.0 * s));
                painter.rect_filled(band, 0.0, egui::Color32::from_rgb(200, 40, 40));
            }
        }
    }

    fn grid_origin() -> egui::Pos2 { egui::pos2(SIDEBAR, TOP_BAR) }
    fn cell_center(col: usize, lane: usize) -> egui::Pos2 {
        let o = Self::grid_origin();
        egui::pos2(o.x + col as f32 * CELL_W + CELL_W / 2.0, o.y + lane as f32 * CELL_H + CELL_H / 2.0)
    }
    ...
    fn nearest_defender_dist(defenders: &[Defender], lane: usize, enemy_x: f32, max_range: f32) -> Option<f32> {
        let grid_x = SIDEBAR;
    ...
    fn update(&mut self, dt: f32) {
        ...
        if self.bonus_msg_timer <= 0.0 { self.bonus_msg_amount = 0; }
        ...
        if !self.enemies.is_empty() {
            self.ink_timer += dt;
            if self.ink_timer >= 1.5 { self.ink_timer -= 1.5; self.ink += 25; }
        }
        ...
        if self.enemies.is_empty() && self.miniwave > 0 && self.miniwave < MINIWAVES && !self.timeskip_given {
            self.miniwave_timer = MINIWAVE_INTERVAL;
            if !self.invested {
                self.ink += 75;
        ...
        if self.wave >= 5 {
            ...
            if self.burst_timer >= 12.0 {
                ...
                for _ in 0..count {
                    let k = if rng.random_range(0.0_f32..1.0) < 0.5 { EnemyKind::BlueScissors } else { EnemyKind::Scissors };
                    let lane = rng.random_range(0..LANES);
                    ...
                    if def.attack_timer >= 0.8 {
                        ...
                        for enemy in &mut self.enemies {
                            if enemy.lane == def.lane
                                && enemy.x > cx - 10.0 && enemy.x < cx + range && enemy.alive()
                            {
                    ...
                    if def.attack_timer >= 1.5 {
                        if self.enemies.iter().any(|e| e.lane == def.lane && e.alive()) {
                            ...
                            new_proj.push(Projectile {
                                lane: def.lane, x: cx, damage: 13.0, pierce: true, hits: 0, is_highlighter: true,
                            });
                        }
                    }
                }
                DefenderKind::Marker => {
                    ...
                    if def.attack_timer >= 3.0 {
                        if self.enemies.iter().any(|e| e.lane == def.lane && e.alive()) {
                            ...
                            let cx = Self::grid_origin().x + def.col as f32 * CELL_W + CELL_W;
                            new_proj.push(Projectile {
                                lane: def.lane, x: cx, damage: 200.0, pierce: false, hits: 0, is_highlighter: false,
                            });
                        }
                    }
                }
        ...
        // Move projectiles
        for p in &mut self.projectiles { p.x += 200.0 * dt; }

        // Projectile-enemy collision
        for proj in &mut self.projectiles {
            for enemy in &mut self.enemies {
                if enemy.lane != proj.lane || (enemy.x - proj.x).abs() >= 15.0 || !enemy.alive() { continue; }
                ...
                proj.hits += 1;
                if !proj.pierce || proj.hits >= 4 { proj.x = -100.0; break; }
                proj.x = enemy.x + 16.0;
            }
        }
        self.projectiles.retain(|p| p.x > 0.0 && p.x < COLS as f32 * CELL_W + SIDEBAR + 100.0);
        ...
        for enemy in &mut self.enemies {
            ...
            match enemy.state {
                EnemyState::Moving => {
                    if enemy.kind.instant_kills() && enemy.kind != EnemyKind::WhiteOut {
                    ...
                    } else if enemy.kind == EnemyKind::WhiteOut || enemy.kind == EnemyKind::ElectronicEraser {
                        enemy.x -= enemy.speed * dt;
                        for def in &mut self.defenders {
                            if def.lane == enemy.lane {
                                let dx = Self::grid_origin().x + def.col as f32 * CELL_W + CELL_W / 2.0;
                        ...
                        if let Some(_dist) = Self::nearest_defender_dist(&self.defenders, enemy.lane, enemy.x, max_range) {
                        ...
                        if blocked { enemy.state = EnemyState::Eating; }
                ...
                EnemyState::Eating => {
                    ...
                    if !still_blocked { enemy.state = EnemyState::Moving; continue; }
                    enemy.attack_timer += dt;
                    if enemy.attack_timer >= 0.8 {
                        enemy.attack_timer = 0.0;
                        enemy.attack_flash = 0.15;
                        let ex = enemy.x; let lane = enemy.lane; let dmg = enemy.damage;
                        if let Some(def) = self.defenders.iter_mut()
                            .filter(|d| {
                                d.lane == lane && {
                                    let dx = Self::grid_origin().x + d.col as f32 * CELL_W + CELL_W / 2.0;
                            ...
                            .min_by(|a, b| {
                                let ax = Self::grid_origin().x + a.col as f32 * CELL_W + CELL_W / 2.0;
                                let bx = Self::grid_origin().x + b.col as f32 * CELL_W + CELL_W / 2.0;
                ...
                EnemyState::HolderStopping(ref mut t) => {
                    *t -= dt;
                    ...
                    let max_range = enemy.kind.attack_range() + CELL_W * 0.7;
                    let has_target = Self::nearest_defender_dist(&self.defenders, enemy.lane, enemy.x, max_range).is_some();
                    if !has_target {
                        enemy.state = EnemyState::Moving;
                    } else if *t <= 0.0 {
                        // Compute extend target based on nearest defender
                        if let Some(dist) = Self::nearest_defender_dist(&self.defenders, enemy.lane, enemy.x, max_range) {
                            enemy.extend_target = (dist - CELL_W * 0.3).max(0.0).min(enemy.kind.attack_range());
                        }
                ...
                EnemyState::HolderExtending => {
                    ...
                    if enemy.extend_offset < enemy.extend_target {
                        enemy.extend_offset = (enemy.extend_offset + speed * dt).min(enemy.extend_target);
                    } else {
                        enemy.extend_offset = (enemy.extend_offset - speed * dt).max(enemy.extend_target);
                    }
                ...
                EnemyState::HolderAttacking => {
                    ...
                    if let Some(dist) = Self::nearest_defender_dist(&self.defenders, enemy.lane, enemy.x, max_range) {
                        let new_target = (dist - CELL_W * 0.3).max(0.0).min(enemy.kind.attack_range());
                    ...
                    if enemy.attack_timer >= 0.8 {
                        ...
                        let lane = enemy.lane; let dmg = enemy.damage;
                        if let Some(def) = self.defenders.iter_mut()
                            .filter(|d| {
                                d.lane == lane && {
                                    let dx = Self::grid_origin().x + d.col as f32 * CELL_W + CELL_W / 2.0;
                ...
                EnemyState::Retreating => {
                    enemy.x += enemy.speed * dt;
                    if enemy.x > COLS as f32 * CELL_W + SIDEBAR { enemy.hp = 0.0; }
                }
            }

            // Loss conditions
            if enemy.kind != EnemyKind::WhiteOut && enemy.state != EnemyState::Retreating
                && !enemy.kind.is_scissors() && enemy.x < SIDEBAR
            {
                self.state = GameState::Lost;
                self.save_score();
                return;
...
impl eframe::App for Game {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        #[cfg(target_arch = "wasm32")]
        { let _ = focused; }
        let dt = if self.paused { 0.0 } else { ctx.input(|i| i.stable_dt).min(0.05) };
        self.update(dt);
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            for lane in 0..LANES {
                for col in 0..COLS {
                    let r = egui::Rect::from_min_size(
                        egui::pos2(grid_o.x + col as f32 * CELL_W, grid_o.y + lane as f32 * CELL_H),
                        egui::vec2(CELL_W, CELL_H),
                    );
                    let c = if (lane + col) % 2 == 0 { egui::Color32::from_rgb(235, 230, 215) }
                        else { egui::Color32::from_rgb(225, 220, 205) };
                    painter.rect_filled(r, 0.0, c);
                    painter.rect_stroke(r, 0.0, egui::Stroke::new(0.5, egui::Color32::from_rgb(200, 195, 180)), egui::StrokeKind::Outside);
                }
            }

            // Top bar
            let bar = egui::Rect::from_min_size(egui::pos2(0.0, 0.0), egui::vec2(SIDEBAR + COLS as f32 * CELL_W, TOP_BAR));
            painter.rect_filled(bar, 0.0, egui::Color32::from_rgb(240, 238, 230));
            painter.rect_stroke(bar, 0.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(200, 195, 180)), egui::StrokeKind::Outside);
            painter.text(egui::pos2(15.0, 20.0), egui::Align2::LEFT_CENTER,
                format!("Ink: {}", self.ink), egui::FontId::proportional(18.0), egui::Color32::from_rgb(30, 30, 40));
            painter.text(egui::pos2(15.0, 42.0), egui::Align2::LEFT_CENTER,
                format!("Wave {}/{}", self.wave, TOTAL_WAVES), egui::FontId::proportional(14.0), egui::Color32::from_rgb(100, 100, 110));
            // Timer
            let mins = (self.time / 60.0) as u32;
            let secs = (self.time % 60.0) as u32;
            let time_str = format!("{}:{:02}", mins, secs);
            painter.text(egui::pos2(SIDEBAR + COLS as f32 * CELL_W - 15.0, 20.0), egui::Align2::RIGHT_CENTER,
                time_str, egui::FontId::proportional(18.0), egui::Color32::from_rgb(30, 30, 40));
            if let Some(hs) = self.high_score {
                let hm = (hs / 60.0) as u32;
                let hs_s = (hs % 60.0) as u32;
                painter.text(egui::pos2(SIDEBAR + COLS as f32 * CELL_W - 15.0, 42.0), egui::Align2::RIGHT_CENTER,
                    format!("Best: {}:{:02}", hm, hs_s), egui::FontId::proportional(14.0), egui::Color32::from_rgb(100, 100, 110));
            }
            ...
            if self.bonus_msg_timer > 0.0 {
                ...
                painter.text(
                    ...
                    egui::Align2::CENTER_CENTER, format!("+{} INK", self.bonus_msg_amount),
            ...
            // Sidebar
            let sidebar_rect = egui::Rect::from_min_size(egui::pos2(0.0, TOP_BAR), egui::vec2(SIDEBAR, LANES as f32 * CELL_H));
            painter.rect_filled(sidebar_rect, 0.0, egui::Color32::from_rgb(215, 210, 200));

            // Defender buttons — sprite + cost
            let kinds = [DefenderKind::Pencil, DefenderKind::Notebook, DefenderKind::Highlighter, DefenderKind::Pen, DefenderKind::Marker];
            let btn_h = 55.0_f32;
            let btn_gap = 4.0_f32;
            for (i, &kind) in kinds.iter().enumerate() {
                let btn = egui::Rect::from_min_size(
                    egui::pos2(4.0, TOP_BAR + 4.0 + i as f32 * (btn_h + btn_gap)), egui::vec2(SIDEBAR - 8.0, btn_h));
                let sel = self.selected == Some(Tool::Place(kind));
                let afford = self.ink >= kind.cost();
                let bg = if sel { egui::Color32::from_rgb(190, 210, 245) }
                    else if afford { egui::Color32::WHITE }
                    else { egui::Color32::from_rgb(195, 193, 188) };
                painter.rect_filled(btn, 4.0, bg);
                painter.rect_stroke(btn, 4.0, egui::Stroke::new(
                    if sel { 2.0 } else { 1.0 },
                    if sel { egui::Color32::from_rgb(60, 80, 160) } else { egui::Color32::from_rgb(150, 145, 135) },
                ...
                painter.text(egui::pos2(btn.max.x - 6.0, btn.center().y), egui::Align2::RIGHT_CENTER,
                    cost_str, egui::FontId::proportional(16.0),
                    if afford { egui::Color32::from_rgb(30, 30, 40) } else { egui::Color32::from_rgb(150, 150, 150) });
                if ui.input(|i| i.pointer.any_pressed()) {
                    if let Some(pos) = ui.input(|i| i.pointer.interact_pos()) {
                        if btn.contains(pos) { self.selected = if sel { None } else { Some(Tool::Place(kind)) }; }
                    }
                }
            }
            ...
            let small_h = 42.0;
            let blob_btn = egui::Rect::from_min_size(egui::pos2(4.0, bottom_y), egui::vec2(SIDEBAR - 8.0, small_h));
            let blob_sel = self.selected == Some(Tool::InkBlob);
            let blob_avail = self.ink_blob_available;
            let blob_bg = if blob_sel { egui::Color32::from_rgb(210, 200, 240) }
                else if blob_avail { egui::Color32::WHITE }
                else { egui::Color32::from_rgb(195, 193, 188) };
            painter.rect_filled(blob_btn, 4.0, blob_bg);
            painter.rect_stroke(blob_btn, 4.0, egui::Stroke::new(if blob_sel { 2.0 } else { 1.0 },
                if blob_sel { egui::Color32::from_rgb(80, 60, 160) } else { egui::Color32::from_rgb(150, 145, 135) }), egui::StrokeKind::Outside);
            ...
            // "x1" or "used" label
            painter.text(egui::pos2(blob_btn.max.x - 6.0, blob_btn.center().y), egui::Align2::RIGHT_CENTER,
                if blob_avail { "x1" } else { "--" }, egui::FontId::proportional(14.0),
                if blob_avail { egui::Color32::from_rgb(30, 30, 40) } else { egui::Color32::from_rgb(150, 150, 150) });
            if ui.input(|i| i.pointer.any_pressed()) {
                if let Some(pos) = ui.input(|i| i.pointer.interact_pos()) {
            ...
            let case_btn = egui::Rect::from_min_size(egui::pos2(4.0, case_y), egui::vec2(SIDEBAR - 8.0, small_h));
            let case_sel = self.selected == Some(Tool::PencilCase);
            let case_bg = if case_sel { egui::Color32::from_rgb(245, 210, 205) } else { egui::Color32::WHITE };
            painter.rect_filled(case_btn, 4.0, case_bg);
            painter.rect_stroke(case_btn, 4.0, egui::Stroke::new(if case_sel { 2.0 } else { 1.0 },
                if case_sel { egui::Color32::from_rgb(200, 80, 60) } else { egui::Color32::from_rgb(150, 145, 135) }), egui::StrokeKind::Outside);
            // Pencil case — zipper on top
            let cc = case_btn.center();
            // Main pouch
            let case_body = egui::Rect::from_center_size(egui::pos2(cc.x, cc.y + 2.0), egui::vec2(50.0, 18.0));
            painter.rect_filled(case_body, 5.0, egui::Color32::from_rgb(140, 110, 75));
            painter.rect_stroke(case_body, 5.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(110, 80, 50)), egui::StrokeKind::Outside);
            // Top flap (slightly lighter)
            let flap = egui::Rect::from_min_size(
                egui::pos2(case_body.min.x, case_body.min.y - 4.0), egui::vec2(50.0, 6.0));
            ...
            // Zipper pull tab
            painter.circle_filled(egui::pos2(cc.x + 6.0, case_body.min.y - 1.0), 2.5, egui::Color32::from_rgb(210, 190, 60));
            painter.circle_stroke(egui::pos2(cc.x + 6.0, case_body.min.y - 1.0), 2.5,
                egui::Stroke::new(0.5, egui::Color32::from_rgb(160, 140, 30)));
            if ui.input(|i| i.pointer.any_pressed()) {
                if let Some(pos) = ui.input(|i| i.pointer.interact_pos()) {
                    if case_btn.contains(pos) { self.selected = if case_sel { None } else { Some(Tool::PencilCase) }; }
                }
            }
                                ...
                                match self.selected {
                                    Some(Tool::Place(kind)) => {
                                        if self.ink >= kind.cost() {
                                            let occ = self.defenders.iter().any(|d| d.lane == lane && d.col == col);
                                            if !occ {
                                                self.ink -= kind.cost();
                                    ...
                                    Some(Tool::InkBlob) => {
                                        if self.ink_blob_available {
                                            ...
                                            for e in &mut self.enemies {
                                                if e.lane == lane && e.x >= cell_left && e.x <= cell_right {
                                                    e.hp = 0.0;
                                                }
                                            }
                                            self.selected = None;
                                    ...
                                    Some(Tool::PencilCase) => {
                                        self.defenders.retain(|d| !(d.lane == lane && d.col == col));
                                    }
                                    None => {}
                                }
                            }
                ...
                if def.attack_flash > 0.0 {
                    ...
                    match def.kind {
                        DefenderKind::Pencil => {
                            let sx = c.x + 28.0;
                            painter.line_segment(
                                [egui::pos2(sx, c.y - 14.0), egui::pos2(sx + 10.0, c.y + 14.0)],
                                egui::Stroke::new(2.5, egui::Color32::from_rgba_premultiplied(80, 80, 90, alpha)));
                        }
                        DefenderKind::Pen => {
                            let sx = c.x + 28.0;
                            painter.line_segment(
                                [egui::pos2(sx, c.y - 14.0), egui::pos2(sx + 10.0, c.y + 14.0)],
                                egui::Stroke::new(2.5, egui::Color32::from_rgba_premultiplied(20, 20, 25, alpha)));
                        }
                        DefenderKind::Marker => {
                            painter.circle_filled(egui::pos2(c.x + 24.0, c.y), 5.0,
            ...
            for proj in &self.projectiles {
                ...
                if proj.is_highlighter {
                    painter.rect_filled(
                        ...
                        1.0, egui::Color32::from_rgb(240, 255, 50));
                } else if proj.damage >= 200.0 {
                    // Marker projectile: red ink blob
                    painter.circle_filled(egui::pos2(proj.x, y), 6.0, egui::Color32::from_rgb(200, 40, 40));
                } else {
                    painter.circle_filled(egui::pos2(proj.x, y), 4.0, egui::Color32::from_rgb(30, 30, 30));
                }
            }
            ...
            for enemy in &self.enemies {
                ...
                match enemy.kind {
                    EnemyKind::PinkEraser => {
                        let r = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(28.0, 20.0));
                        painter.rect_filled(r, 4.0, egui::Color32::from_rgb(240, 150, 170));
                        painter.rect_stroke(r, 4.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(200, 100, 130)), egui::StrokeKind::Outside);
                        painter.rect_filled(egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(6.0, 20.0)),
                            0.0, egui::Color32::from_rgb(220, 120, 145));
                    }
                    EnemyKind::WhitePolymer => {
                        let r = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(28.0, 20.0));
                        painter.rect_filled(r, 4.0, egui::Color32::from_rgb(240, 240, 245));
                        painter.rect_stroke(r, 4.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(180, 180, 190)), egui::StrokeKind::Outside);
                    }
                    EnemyKind::WrappedPolymer => {
                        if enemy.shell_hp > 0.0 {
                            let outer = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(34.0, 26.0));
                            painter.rect_filled(outer, 2.0, egui::Color32::from_rgb(160, 130, 90));
                            painter.rect_stroke(outer, 2.0, egui::Stroke::new(1.5, egui::Color32::from_rgb(120, 95, 60)), egui::StrokeKind::Outside);
                            let inner = egui::Rect::from_center_size(egui::pos2(x, y - 2.0), egui::vec2(26.0, 14.0));
                    ...
                    EnemyKind::BlackEraser => {
                        ...
                        painter.rect_stroke(r, 4.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(20, 20, 25)), egui::StrokeKind::Outside);
                    }
                    EnemyKind::WrappedBlackEraser => {
                        if enemy.shell_hp > 0.0 {
                            let outer = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(36.0, 28.0));
                            painter.rect_filled(outer, 2.0, egui::Color32::from_rgb(80, 30, 30));
                            painter.rect_stroke(outer, 2.0, egui::Stroke::new(2.0, egui::Color32::from_rgb(50, 15, 15)), egui::StrokeKind::Outside);
                            let inner = egui::Rect::from_center_size(egui::pos2(x, y - 2.0), egui::vec2(28.0, 16.0));
                    ...
                    EnemyKind::EraserHolder | EnemyKind::BlueEraserHolder => {
                        let barrel = egui::Rect::from_center_size(egui::pos2(x + 4.0, y), egui::vec2(32.0, 10.0));
                        painter.rect_filled(barrel, 3.0, egui::Color32::from_rgb(35, 35, 40));
                        painter.rect_stroke(barrel, 3.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(20, 20, 25)), egui::StrokeKind::Outside);
                        painter.rect_filled(egui::Rect::from_center_size(egui::pos2(x + 8.0, y), egui::vec2(4.0, 12.0)),
                            1.0, egui::Color32::from_rgb(80, 80, 85));
                        painter.rect_filled(egui::Rect::from_center_size(egui::pos2(x + 22.0, y), egui::vec2(6.0, 8.0)),
                            1.0, egui::Color32::from_rgb(60, 60, 65));
                        ...
                        let tip = egui::Rect::from_min_size(
                            egui::pos2(x - 12.0 - ext, y - 4.0), egui::vec2(tip_len, 8.0));
                        painter.rect_filled(tip, 2.0, tip_color);
                        painter.rect_stroke(tip, 2.0, egui::Stroke::new(0.5, egui::Color32::from_rgb(180, 180, 190)), egui::StrokeKind::Outside);
                    }
                    EnemyKind::Scissors | EnemyKind::BlueScissors => {
                        let (blade_c, handle_c, handle_stroke, sz) = if enemy.kind == EnemyKind::BlueScissors {
                            (egui::Color32::from_rgb(100, 140, 220), egui::Color32::from_rgb(50, 80, 180),
                             egui::Color32::from_rgb(30, 50, 130), 1.2_f32)
                        } else {
                            (egui::Color32::from_rgb(180, 180, 190), egui::Color32::from_rgb(200, 60, 60),
                             egui::Color32::from_rgb(150, 40, 40), 1.0_f32)
                        };
                        let s = sz;
                        painter.line_segment([egui::pos2(x - 14.0*s, y - 10.0*s), egui::pos2(x + 14.0*s, y + 10.0*s)],
                            egui::Stroke::new(3.0*s, blade_c));
                        painter.line_segment([egui::pos2(x - 14.0*s, y + 10.0*s), egui::pos2(x + 14.0*s, y - 10.0*s)],
                            egui::Stroke::new(3.0*s, blade_c));
                        // Handles on RIGHT (rear)
                        painter.circle_filled(egui::pos2(x + 14.0*s, y - 10.0*s), 5.0*s, handle_c);
                        painter.circle_filled(egui::pos2(x + 14.0*s, y + 10.0*s), 5.0*s, handle_c);
                        painter.circle_stroke(egui::pos2(x + 14.0*s, y - 10.0*s), 5.0*s, egui::Stroke::new(1.0, handle_stroke));
                        painter.circle_stroke(egui::pos2(x + 14.0*s, y + 10.0*s), 5.0*s, egui::Stroke::new(1.0, handle_stroke));
                    }
                    EnemyKind::WhiteOut => {
                        let body = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(30.0, 16.0));
                        painter.rect_filled(body, 3.0, egui::Color32::from_rgb(245, 245, 250));
                        painter.rect_stroke(body, 3.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(180, 180, 190)), egui::StrokeKind::Outside);
                        let tip = vec![
                            egui::pos2(body.min.x, y - 3.0),
                            egui::pos2(body.min.x - 8.0, y),
                            egui::pos2(body.min.x, y + 3.0),
                        ];
                        painter.add(egui::Shape::convex_polygon(tip, egui::Color32::from_rgb(220, 220, 225), egui::Stroke::NONE));
                        painter.circle_filled(egui::pos2(x + 4.0, y - 2.0), 5.0, egui::Color32::from_rgb(230, 230, 235));
                        painter.circle_stroke(egui::pos2(x + 4.0, y - 2.0), 5.0, egui::Stroke::new(0.5, egui::Color32::from_rgb(180, 180, 190)));
                        painter.rect_filled(
                            egui::Rect::from_center_size(egui::pos2(x + 2.0, y + 4.0), egui::vec2(16.0, 3.0)),
                            1.0, egui::Color32::from_rgb(50, 130, 200));
                    }
                    EnemyKind::KneadedEraser => {
                        ...
                        painter.rect_stroke(r, 8.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(120, 120, 125)), egui::StrokeKind::Outside);
                    }
                    EnemyKind::ShinyPlastic | EnemyKind::LargeShinyPlastic => {
                        let sz = if enemy.kind == EnemyKind::LargeShinyPlastic { 1.5 } else { 1.0_f32 };
                        let r = egui::Rect::from_center_size(egui::pos2(x, y), egui::vec2(26.0 * sz, 18.0 * sz));
                        painter.rect_filled(r, 4.0, egui::Color32::from_rgba_premultiplied(160, 220, 240, 180));
                        painter.rect_stroke(r, 4.0, egui::Stroke::new(1.0, egui::Color32::from_rgba_premultiplied(100, 180, 220, 200)), egui::StrokeKind::Outside);
                        painter.line_segment(
                            [egui::pos2(r.min.x + 4.0, r.min.y + 3.0), egui::pos2(r.max.x - 8.0, r.min.y + 3.0)],
                            egui::Stroke::new(1.5, egui::Color32::from_rgba_premultiplied(255, 255, 255, 140)));
                    }
                    EnemyKind::ElectronicEraser => {
                        // Toothbrush-shaped body (handle)
                        let handle = egui::Rect::from_center_size(egui::pos2(x + 4.0, y), egui::vec2(24.0, 8.0));
                        painter.rect_filled(handle, 3.0, egui::Color32::from_rgb(180, 180, 190));
                        painter.rect_stroke(handle, 3.0, egui::Stroke::new(0.5, egui::Color32::from_rgb(140, 140, 150)), egui::StrokeKind::Outside);
                        // Neck (thinner part connecting handle to head)
                        let neck = egui::Rect::from_center_size(egui::pos2(x - 10.0, y), egui::vec2(6.0, 6.0));
                        ...
                        let steps = 8;
                        for i in 0..steps {
                            let a1 = std::f32::consts::PI * 0.5 + std::f32::consts::PI * (i as f32 / steps as f32);
                            let a2 = std::f32::consts::PI * 0.5 + std::f32::consts::PI * ((i + 1) as f32 / steps as f32);
                            let tri = vec![
                                egui::pos2(fan_cx, y),
                                egui::pos2(fan_cx + fan_r * a1.cos(), y + fan_r * a1.sin()),
                                egui::pos2(fan_cx + fan_r * a2.cos(), y + fan_r * a2.sin()),
                            ];
                            painter.add(egui::Shape::convex_polygon(tri, fan_color, egui::Stroke::NONE));
                        }
                        // Fan edge outline
                        painter.circle_stroke(egui::pos2(fan_cx, y), fan_r, egui::Stroke::new(1.0, egui::Color32::from_rgb(40, 40, 50)));
                        // Button on handle
                        painter.circle_filled(egui::pos2(x + 10.0, y), 2.5, egui::Color32::from_rgb(50, 180, 230));
                ...
                if enemy.hit_flash > 0.0 {
                    ...
                    painter.line_segment(
                        ...
                        egui::Stroke::new(2.5, egui::Color32::from_rgba_premultiplied(60, 60, 70, alpha)));
                }
                ...
                if enemy.attack_flash > 0.0 && !enemy.kind.instant_kills() {
                    ...
                    painter.line_segment(
                        [egui::pos2(x - 18.0, y - 12.0), egui::pos2(x - 5.0, y + 12.0)],
                        egui::Stroke::new(2.5, egui::Color32::from_rgba_premultiplied(220, 20, 20, alpha)));
                }
            }

            // Pause overlay
            if self.paused && self.state == GameState::Playing {
                let screen = egui::Rect::from_min_size(egui::pos2(0.0, 0.0), egui::vec2(SIDEBAR + COLS as f32 * CELL_W, TOP_BAR + LANES as f32 * CELL_H));
                painter.rect_filled(screen, 0.0, egui::Color32::from_rgba_premultiplied(0, 0, 0, 100));
                painter.text(screen.center(), egui::Align2::CENTER_CENTER, "Paused",
                    egui::FontId::proportional(40.0), egui::Color32::WHITE);
            }
            ...
            if self.state != GameState::Playing {
                ...
                let msg = if self.state == GameState::Won { "You Win!" } else { "Game Over" };
                painter.text(screen.center(), egui::Align2::CENTER_CENTER, msg,
                    egui::FontId::proportional(48.0), egui::Color32::WHITE);
                // Show time
                let mins = (self.time / 60.0) as u32;
                let secs = (self.time % 60.0) as u32;
                ...
                painter.text(egui::pos2(screen.center().x, screen.center().y + 40.0),
                    egui::Align2::CENTER_CENTER, format!("Time: {}:{:02}", mins, secs),
                    egui::FontId::proportional(22.0), sub_color);
                if let Some(hs) = self.high_score {
                    let hm = (hs / 60.0) as u32;
                    let hs_s = (hs % 60.0) as u32;
                    painter.text(egui::pos2(screen.center().x, screen.center().y + 65.0),
                        egui::Align2::CENTER_CENTER, format!("Best: {}:{:02}", hm, hs_s),
                        egui::FontId::proportional(16.0), sub_color);
                }
                painter.text(egui::pos2(screen.center().x, screen.center().y + 90.0),
                    egui::Align2::CENTER_CENTER, "Click to restart",
                    egui::FontId::proportional(18.0), sub_color);
                if ui.input(|i| i.pointer.any_pressed()) { *self = Game::new(); }
            }
        });
